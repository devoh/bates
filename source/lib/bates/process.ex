defmodule Bates.Process do
  ...
  defp assign_port(%__MODULE__{port: 0} = process),
    do: %{process | port: Bates.PortNumber.next()}

  defp assign_port(process), do: process

  defp broadcast(name, message) do
    Phoenix.PubSub.broadcast(Bates.PubSub, "process:#{name}", message)
  end
  ...
  defp via_tuple(%__MODULE__{name: name}) when not is_nil(name),
    do: {:via, Registry, {Bates.ProcessRegistry, name}}
